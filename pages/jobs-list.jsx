...
export default function JobsListPage() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        setLoading(true);
        const data = await api.jobApi.getJobs();
        console.log("JobsListPage: fetched jobs:", data);
        // Transform backend data to match frontend expectations if needed
        const transformedJobs = data.map(job => {
          // Compute salary string
          ...
          if (job.salary_min || job.salary_max) {
            ...
            salary = job.salary_min && job.salary_max ? `${min} - ${max}` : (min || max);
          }
          
          // Get company name from nested company object
        ...
        console.log("JobsListPage: transformed jobs:", transformedJobs);
  ...
  const badgeStyle = (type) => {
    if(type.includes("Full Time")) return { background:"#dcfce7",color:"#166534" };
    if(type==="Contract") return { background:"#fee2e2",color:"#dc2626" };
  ...
  if (jobs.length === 0) {
    return (
      <div style={{ padding: 80, textAlign: "center" }}>
        ...
        <button onClick={() => router.push("/auth/signup")} style={{ padding: "12px 24px", background: "#2563eb", color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontWeight: 600 }}>
          Create Account
        </button>
      </div>
    );
            ...
            <div style={{ display:"grid",gridTemplateColumns:"5fr 3fr 3fr 1fr",gap:12 }}>
              {[
                ...
                <label key={i} style={{ display:"flex",alignItems:"center",gap:10,background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:10,padding:"0 14px",height:50 }}>
                  ...
                  {f.type==="text" ? <input type="text" placeholder={f.placeholder} style={{ border:"none",background:"transparent",outline:"none",fontSize:13,flex:1 }}/> :
          ...
          <div style={{ display:"grid",gridTemplateColumns:"280px 1fr",gap:24 }}>
            ...
            <div>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
                {jobs.map((job, i) => (
                  <div key={i} style={{ background: job.featured?"#f0f7ff":"#fff",borderRadius:16,border:`1px solid ${job.featured?"#2563eb":"#e5e7eb"}`,padding:24,transition:"all 0.3s" }}
                    onMouseEnter={e=>e.currentTarget.style.boxShadow="0 10px 40px rgba(0,0,0,.08)"}
                    onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                    <div style={{ marginBottom:14 }}>
                      <span style={{ ...badgeStyle(job.type),display:"inline-block",padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:600 }}>{job.type}</span>
                    </div>
                    <div style={{ display:"flex",gap:20,alignItems:"flex-start" }}>
                       <div style={{ width:70,height:70,borderRadius:12,overflow:"hidden",flexShrink:0 }}>
                         {job.logo ? (
                           <img 
                             ...
                             onError={(e) => {
                               e.target.src = 'https://via.placeholder.com/70?text=Logo';
                      ...
                      <div style={{ flex:1 }}>
                        <h4 style={{ fontSize:18,fontWeight:700,color:"#1f2937",marginBottom:8 }}>
                          <a onClick={()=>router.push(`/job-single/${job.id}`)} style={{ color:"inherit",cursor:"pointer",textDecoration:"none" }}>{job.title}</a>
                        </h4>
                      ...
                      <div style={{ display:"flex",flexDirection:"column",gap:8,alignItems:"flex-end",flexShrink:0 }}>
                         <button onClick={()=>router.push(`/job-single/${job.id}`)} style={{ padding:"10px 20px",border:"1px solid #17d27c",color:"#17d27c",background:"transparent",borderRadius:8,cursor:"pointer",fontSize:14,fontWeight:600 }}>View Details</button>
